...
noise.settings(1, 2.5, 1, 0.8, 32, 3, 1);
...
let structures = [
    ...
    createSphereMeshVector3(4096, 0.5, 0, 0, 0),
];
...
let textures = [
    ...
    new Texture2D(
        16, 16,
        Array.from(noise.perlinBuffer(16, 16, 0.5, 0.5)).map((noiseVal) => {
            let adjustedNoiseVal = noiseVal * 0.5 + 0.5;

            let r = adjustedNoiseVal * 255;
            let g = adjustedNoiseVal * 255;
            let b = adjustedNoiseVal * 255;
            let a = 255;
...
function loadImageToTexture(img, textureIndex) {
    try {
        img.crossOrigin = "";
        ...
        newCtx.drawImage(img, 0, 0, img.width, img.height, 0, 0, img.width, img.height);
        let data = newCtx.getImageData(0, 0, img.width, img.height);

        textures[textureIndex] = new Texture2D(img.width, img.height, data.data);
                ...
                for (let x = 0; x < rangeX; x++) {
                    ...
                    for (let y = 0; y < rangeY; y++) {
                        ...
                        if (w1 >= 0 && w2 >= 0 && w3 >= 0) {
                            ...
                            let z = 1 / (w1 + w2 + w3);
                            let x_1 = (x + minX) * -z / zNear;
                            let y_1 = (y + minY) * -z / zNear;
